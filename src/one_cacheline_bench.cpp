...
alignas(64) static std::atomic<int> ping_pong{-1};
...
static void WorkerThread(int cpu_id, int num_samples, int num_inner_rounds) {
    ...
    for (int sample = 0; sample < num_samples; ++sample) {
        for (int round = 0; round < num_inner_rounds; ++round) {
            while (ping_pong.load(std::memory_order_acquire) != (round * 2 + 1)) {
                // Busy wait
            }

            ping_pong.store(round * 2 + 2, std::memory_order_release);
        }
    }
}
...
std::vector<uint64_t> Measure(int core1_id, int core2_id, int num_inner_rounds) {
    ...
    for (int sample = 0; sample < num_samples; ++sample) {
        ...
        for (int round = 0; round < num_inner_rounds; ++round) {
            ping_pong.store(round * 2 + 1, std::memory_order_release);

            while (ping_pong.load(std::memory_order_acquire) != (round * 2 + 2)) {
                // Busy wait
            }
        }
